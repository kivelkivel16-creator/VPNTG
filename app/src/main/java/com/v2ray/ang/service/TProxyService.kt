...
class TProxyService(
    ...
    override fun startTun2Socks() {
        ...
        Log.i(AppConfig.TAG, "HevTun: Starting with config:\n$configContent")
        Log.i(AppConfig.TAG, "HevTun: VPN fd=${vpnInterface.fd}")

        try {
            TProxyStartService(configFile.absolutePath, vpnInterface.fd)
            Log.i(AppConfig.TAG, "HevTun: TProxyStartService called successfully")
        } catch (e: Exception) {
            Log.e(AppConfig.TAG, "HevTun: TProxyStartService FAILED: ${e.message}")
        }
    }
