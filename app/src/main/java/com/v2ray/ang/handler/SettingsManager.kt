...
//        }
    ...
    /**
     * Check if per-app proxy (allowlist) mode is active.
     * In this mode xray runs as SOCKS-only proxy — no TUN inbound needed.
     */
    fun isPerAppProxyMode(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_PER_APP_PROXY) &&
                !MmkvManager.decodeSettingsBool(AppConfig.PREF_BYPASS_APPS)
    }
    ...
    private fun ensureDefaultSettings() {
        // HEV tunnel is required to bridge the VPN interface to the SOCKS proxy
        // libhev-socks5-tunnel.so is bundled in app/libs/arm64-v8a/
