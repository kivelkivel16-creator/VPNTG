...
//        if (!result.status) return
    ...
    fun startCoreLoop(vpnInterface: ParcelFileDescriptor?): Boolean {
        ...
        val configStr = result.content
        configStr.chunked(1000).forEachIndexed { i, chunk ->
            Log.i(AppConfig.TAG, "Config[$i]: $chunk")
        }
        ...
        if (SettingsManager.isUsingHevTun() || SettingsManager.isPerAppProxyMode()) {
            tunFd = 0
        }
